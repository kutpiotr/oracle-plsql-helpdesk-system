CREATE OR REPLACE PACKAGE BODY ticket_management_pkg AS
...
RAISE_APPLICATION_ERROR(-20001, 'Ticket title cannot be empty.');
END IF;

IF p_description IS NULL OR DBMS_LOB.GETLENGTH(p_description) = 0 THEN
RAISE_APPLICATION_ERROR(-20002, 'Ticket description cannot be empty.');
END IF;

IF p_priority NOT IN ('LOW', 'MEDIUM', 'HIGH') THEN
RAISE_APPLICATION_ERROR(-20003, 'Invalid ticket priority.');
END IF;
...
RAISE_APPLICATION_ERROR(-20004, 'Creating user does not exist.');
END IF;
